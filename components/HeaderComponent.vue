<template>
	<header class="flex items-center justify-between px-5 h-24 w-full bg-charcoal">
		...
		<div class="hidden gap-2 items-center md:flex">
			<button @click="toggleFormModal(true)" class="hidden md:flex gap-2 items-center bg-savoy rounded-3xl px-5 py-3 font-semibold hover:scale-105 transition">
				+ Добавить Карточку
			</button>
			<EllipsisVerticalIcon class="w-10 h-10 cursor-pointer" @click="() => (editBoardFormState = true)" />
		</div>
		<Bars2Icon class="md:hidden w-10 h-10" @click="() => (isMobileMenuOpen = true)" />
		...
		<transition name="fade">
			<div v-if="isMobileMenuOpen" class="center-fixed w-full h-full bg-neutral-800 flex flex-col gap-10 items-center justify-center font-bold z-20">
				...
				<XMarkIcon class="w-10 h-10 absolute top-8 right-5 z-10" @click="() => (isMobileMenuOpen = false)" />
			</div>
		</transition>
	</header>
</template>
